    ...
    const onClickSaveBtn = async () => {
        try {
            if (setTotalLocationList && day !== undefined && day >= 0) {
                ...
                const originalLocation = totalLocationList[day][locationIndex - 1];
                const updatedImages = [...(originalLocation.images || [])];
                uploadedImages.forEach(img => {
                    if (img) {
                        updatedImages[img.index] = img.url;
                    }
                });
                ...
                const formData = new FormData();
                formData.append('locationUpdateReqDto', new Blob([JSON.stringify(updatedLocationData)], { type: "application/json" }));

                await axios.put(`/api/location/${locationId}`, formData);

                setTotalLocationList(prevTotalLocationList => {
                    const newTotalLocationList = prevTotalLocationList.map(dayLocations => [...dayLocations]);
                    newTotalLocationList[day][locationIndex - 1] = updatedLocationData;
